...
echo "Jupyterhub IP: $(kubectl --namespace="${HELM_BINDERHUB_NAME}" get svc proxy-public | awk '{ print $4}' | tail -n 1)"
echo "Binderhub IP: $(kubectl --namespace="${HELM_BINDERHUB_NAME}" get svc binder | awk '{ print $4}' | tail -n 1)"
